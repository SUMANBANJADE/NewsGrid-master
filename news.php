...
<form action="submit-comment.php" method="POST" class="comment-form">
    ...
    <input type="text" name="user_name" placeholder="Your Name" class="comment-input" required><br><br>

    <input type="email" name="email" placeholder="Your Email" class="comment-input" required><br><br>

    <textarea name="comment" placeholder="Write your comment..." class="comment-textarea" required></textarea><br><br>
...
$comment_query = "SELECT * FROM comments WHERE article_id = $article_id ORDER BY created_at DESC";
$comment_result = mysqli_query($con, $comment_query);

while ($row = mysqli_fetch_assoc($comment_result)) {
    echo "<div class='comment-box'>";
    echo "<strong>" . htmlspecialchars($row['user_name']) . "</strong> (" . $row['created_at'] . ")<br>";
